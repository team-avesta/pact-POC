...
const providerName = "Jest-Provider-Example";
const consumerName = "Jest-Consumer-Example";

//const pactFile = path.resolve(`./pact/pacts/${consumerName}-${providerName}.json`);
process.env.PACT_URL =
    "http://localhost:9292/pacts/provider/Jest-Provider-Example/consumer/Jest-Consumer-Example/latest";

process.env.PACT_BROKER_BASE_URL = "http://localhost:9292";
process.env.PACT_BROKER_TOKEN = "5gFImWvd4sXY280BZpfh2A";
let port;
let opts;
let app;
        ...
        if (process.env.PACT_BROKER_BASE_URL) {
            ...
            if (!process.env.PACT_URL) {
                opts = {
                    ...
                    consumerVersionSelectors: [{ mainBranch: true }, { deployedOrReleased: false }],
